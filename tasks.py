...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_CLEMEA24_xxx
namespace = Collection("clemea24")
namespace.configure(
    {
        "clemea24": {
            "nautobot_ver": "2.1.2",
            "project_name": "clemea24",
            "python_ver": "3.11",
            "local": False,
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.clemea24.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.clemea24.compose_http_timeout,
        "NAUTOBOT_VER": context.clemea24.nautobot_ver,
        "PYTHON_VER": context.clemea24.python_ver,
        **kwargs.pop("env", {}),
    }
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.clemea24.project_name}",
        f'--project-directory "{context.clemea24.compose_dir}"',
    ]

    for compose_file in context.clemea24.compose_files:
        compose_file_path = os.path.join(context.clemea24.compose_dir, compose_file)
...
def run_command(context, command, **kwargs):
    ...
    if is_truthy(context.clemea24.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.clemea24.python_ver}...")
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations clemea24"
...
@task(
    help={
        "version": "Version of Clemea24 to generate the release notes for.",
    }
)
...
def pylint(context):
    ...
    command = 'pylint --init-hook "import nautobot; nautobot.setup()" --rcfile pyproject.toml clemea24'
    run_command(context, command)
...
def unittest_coverage(context):
    ...
    command = "coverage report --skip-covered --include 'clemea24/*' --omit *migrations*"
...
def tests(context, failfast=False, keepdb=False, lint_only=False):
    ...
    if not is_truthy(context.clemea24.local):
